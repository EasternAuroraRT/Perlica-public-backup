...
class BioSimEngine:
    ...
    def __init__(self, config: Optional[BiorythmConfig] = None, start_hour: float = 8.0,
        ...
        :param time_scale: 时间倍率（模拟时间 / 现实时间），1.0 表示与现实时间同步
        """
        self.cfg = config if config else BiorythmConfig()
        ...
        # 时间基准：以单调时钟为轴，模拟时间 = 分段累加的（实测现实时长 × 倍率）
    ...
    def stop(self):
        ...
        self._thread = None
        with self._lock:
            # 结算最后一个未走完的 tick 尾段，使模拟时间与单调时钟精确对齐
            target_sim_hours = self._sim_hours_now()
            delta = target_sim_hours - self._last_sim_hours
            if delta > 0:
                self.machine.advance_time(delta)
                self._last_sim_hours = target_sim_hours

    def _sim_hours_now(self) -> float:
        """当前应推进到的模拟小时数（基于单调时钟，与睡眠误差无关）"""
    ...
    def _run(self):
        while not self._stop_event.is_set():
            ...
            if self._stop_event.is_set():
                break
            with self._lock:
                # 用单调时钟实测的真实经过时间求目标时刻，再推进差值：
                # 每次推进量 = 实际流逝时长 × 倍率，理论零累计误差
                target_sim_hours = self._sim_hours_now()
                delta = target_sim_hours - self._last_sim_hours
                if delta > 0:
                    self.machine.advance_time(delta)
...
if __name__ == "__main__":
    ...
    # 创建引擎，time_scale=360 表示每秒推进 0.1 模拟小时（6 分钟）
    engine = BioSimEngine(config=fast_config, start_hour=8.0,
                              update_interval=0.5, time_scale=360)
    ...
    try:
        ...
        while True:
            ...
            # 演示动态调整倍率（循环中加速再减速）
            if cycle == 5:
                print("\n>>> 加速：time_scale = 1800 (每秒推进 0.5 模拟小时)")
                engine.set_time_scale(1800)
            elif cycle == 10:
                print("\n>>> 减速：time_scale = 180 (每秒推进 0.05 模拟小时)")
                engine.set_time_scale(180)
            elif cycle == 15:
                print("\n>>> 恢复初始节奏：time_scale = 360")
                engine.set_time_scale(360)
