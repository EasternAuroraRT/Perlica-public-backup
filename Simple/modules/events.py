                ...
                case np.PokeEvent(): # pyright: ignore[reportGeneralTypeIssues]
                    ...
                    match event:
                        ...
                        case np.GroupPokeEvent():
                            ...
                            prompt = ('You were' if event.target_id == env.self_id else f"User `{target_nickname}` was") + ' poked in group ' + f'{event.group_id}' + ' in the chatbox. Interaction msg: ' + parse_poke_raw(event.raw_info, sender_nickname, '你' if event.target_id == env.self_id else target_nickname)
                    api_call_msg.append({'type': 'text', 'text': prompt})
